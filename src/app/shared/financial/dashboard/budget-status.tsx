  ...
  return (
    <div>
      <div className="relative flex h-10 flex-col justify-end overflow-hidden rounded-[10px] bg-gray-100">
        <div
          className="absolute bottom-0 left-0 top-0 z-10 h-full bg-gradient-to-r from-[#477DFF] to-[#96C5FC]"
        ...
        <div className="relative z-20 mt-auto flex w-full items-end justify-between [&>:nth-child(10n+1)]:!h-4">
          {Array.from({ length: 101 }).map((_, index) => (
            <span
              key={index}
              className="h-2.5 w-[1px] bg-gray-500 dark:bg-gray-300"
            />
          ))}
        </div>
      </div>
  ...
  return (
    <div className="group">
      ...
      <div className="relative h-2.5 w-full rounded-lg bg-gray-100">
        <div
          style={{ width: `${percentage}%` }}
          className="h-full rounded-lg bg-gradient-to-r from-[#477DFF] to-[#96C5FC]"
        />
        <div className="absolute left-0 top-1/2 flex h-0 w-full -translate-y-1/2 items-center bg-black/50">
          <div className="max-auto relative w-full">
            <Tooltip
              tooltipArrowClassName="dark:fill-gray-200"
            ...
            >
              <span
                ...
                className="absolute top-1/2 block h-5 w-5 -translate-x-1/2 -translate-y-1/2 scale-75 cursor-pointer rounded-full border-[5.5px] border-gray-0  bg-[#477DFF] opacity-0 shadow-md duration-100 group-hover:scale-100 group-hover:opacity-100 dark:border-gray-200"
              />
            </Tooltip>
          </div>
        </div>
...
export default function BudgetStatus({ className }: { className?: string }) {
  return (
    <WidgetCard
      ...
      actionClassName="ps-0 mb-2 @[30rem]:mb-4"
      titleClassName="text-gray-700 font-normal sm:text-base font-inter mb-3 @[30rem]:mb-7"
