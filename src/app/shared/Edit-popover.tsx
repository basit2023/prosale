...
type UserType = {
  user: {
    ...
    sms: string;
    lead_status: string;
    name: string; // Ensure this is included if `name` is part of the `user` object
  };
};
        ...
        return (
          <>
            ...
            <table className="border-collapse border border-dashed border-slate-400 ...">
              ...
              <tbody>
                <tr className='justify-center'>
                  ...
                  <td className="border border-dashed border-slate-300 text-center bg-gray-200 h-8 ...">{ <Controller
                  control={control}
                  name="sms"
                  render={({ field: { value, onChange } }:any) => (
                   <SelectBox
                  //  value={value?.user?.sms === 'Y' ? 'Enabled' : 'Disabled'}
                    placeholder="Enabled"
                    options={status}
                    onChange={onChange}
                    // @ts-ignore
                    value={value}
                    className=" p-0 m-1 bg-transparent text-sm max-w-xs"  // Adjusted padding and added text size
                    getOptionValue={(option) => option.value}
                    displayValue={(selected) =>
                      status?.find((r) => r.value === selected)?.name ?? ''
                    }
                ...
                <tr className='justify-center'>
                  ...
                  <td className="border border-dashed border-slate-300 text-center bg-gray-100 h-8 ...">{ <Controller
                  control={control}
                  name="lead_status"
            ...
            <div className="mb-10 w-720px border border-dashed border-gray-200 pl-4 pr-4 ">
              {/* FormFooter styling adjusted to match other elements */}
              <FormFooter altBtnText="Cancel" submitBtnText="Update" className="border-t border-dashed border-gray-200 pt-2 pb-0" onCancel={closeModal} />
            </div>
          </>
        );
      }}
