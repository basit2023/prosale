...
import CreateNewEmployee from './AddnewEmployee/AddNewEmployee';
...
export const FileInput = ({
  ...
  function handleFileUpload() {
    if (files.length) {
      ...
      // Create a folder path on the frontend
      const uploadFolder = '/assets'; // Update this to your desired folder structure
  
      // Process each file on the frontend and store it
      files.forEach((file, index) => {
        ...
        reader.onload = () => {
          ...
          // Create a Blob with the same content and type
          const blob = new Blob([content], { type: file.type });
  
          // Create a new file with the Blob
          const newFile = new File([blob], `${uploadFolder}/${file.name}`, { type: file.type });
  
          // You can use the newFile as needed, for example, display it or store it in state
          console.log('Storing file on the frontend:', newFile);
  
          // Example: set the new file in state
          setFiles((prevFiles) => [...prevFiles, newFile]);
  
          // Example: Display the file content (this is just for demonstration)
          const fileContent = new TextDecoder().decode(new Uint8Array(content as ArrayBuffer));
      ...
      toast.success(<Text as="b">Files successfully stored on the frontend</Text>);
    } else {
