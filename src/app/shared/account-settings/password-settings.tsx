...
export default function PasswordSettingsView({
  ...
  const onSubmit: SubmitHandler<PasswordFormTypes> = async (data) => {
    ...
    if(currentP==`${value?.user?.password}`){
    setLoading(true);
    setTimeout(async () => { 
      setLoading(false);
     
      try {
        const hashedInput = crypto.createHash('sha256').update(data.confirmedPassword).digest('hex');
        await apiService.put(`/forget-password/${session?.user?.email}`, { password: hashedInput });
        if(Response){
          logs({ user: value?.user?.name, desc: 'Password' });
        }
    ...
    toast.error('Current password is not correct. Please try again.');
