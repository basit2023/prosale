...
export default function AddNewProjectfloor({slug}:any) {
  ...
  const onSubmit: SubmitHandler<NewFloorFormTypes> = async (data) => {
    ...
    try {
      ...
      if(company?.user_data?.number<=1){
        ...
        const result = await apiService.post(`/create-new-floor`, {
          ...data,user: value?.user?.name, slug:slug
        });
        console.log("the result of the data is:",data)
